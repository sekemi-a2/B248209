...
```
Greater Glasgow and Clyde, ranked as the third most deprived region, exhibits the largest increase in Type 2 prescriptions per head. Lothian and Lanarkshire also show large increases, while boards such as Shetland and Orkney show smaller increases. 

However, absolute increases are heavily influenced by population size. To assess the relative severity of change across boards, percentage increases were also examined.
```{r message=FALSE, warning=FALSE}
...
interactive_map
#Add click to see name of healthboard and exact percentage number
```
